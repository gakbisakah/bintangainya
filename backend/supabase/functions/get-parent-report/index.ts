...
serve(async (req) => {
  ...
  try {
    ...
    const scoresText = submissions?.map((s: any) => `- ${s.assignments?.title || 'Tugas'}: ${s.total_score}`).join("\n") || "Belum ada tugas.";
    const topicsText = student.weak_topics?.length > 0 ? student.weak_topics.join(", ") : "Tidak ada topik spesifik.";

    // 3. Panggil AI Service untuk narasi
    const systemPrompt = `Kamu adalah Kak BintangAi, asisten pelaporan pendidikan.
    Tugasmu adalah membuat laporan mingguan yang ramah, profesional, dan mudah dimengerti orang tua.
    Gunakan Bahasa Indonesia yang hangat. Maksimal 150 kata.`;
    ...
    console.error("Get-Parent-Report Error:", error);
